...
const Setting = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get("generalSetting/", {
          headers: {},
        });
        ...
        if (response.data && response.data.data) {
          ...
          const data =
            Array.isArray(dataArr) && dataArr.length > 0 ? dataArr[0] : {};
          setInitialValues({
            add: data.add || "",
            deduct: data.deduct || "",
        ...
        {({ values, handleChange, handleBlur, handleSubmit, validateForm }) => (
          <form>
            ...
            <div className="row mb-3 my-5">
              <label className={`col-sm-2 ${styles.titleDay} `}>
                {" "}
                اليوم الاجازة الرسمي 1:
              </label>
              <div className={`col-sm-10  `}>
                <select
                  name="offDay1"
                  className={`form-select ${styles.formSelect2}`}
                  value={values.offDay1}
                  onChange={handleChange}
                  onBlur={handleBlur}
                >
            ...
            <div className="row mb-3">
              <label className={`col-sm-2  ${styles.titleDay} `}>
                {" "}
                اليوم الاجازة الرسمي 2:{" "}
              </label>
              <div className={`col-sm-10 `}>
                <select
                  name="offDay2"
                  className={`form-select ${styles.formSelect2}`}
                  value={values.offDay2}
                  onChange={handleChange}
                  onBlur={handleBlur}
                >
            ...
            <div className="text-center">
              <button
                ...
                onClick={() =>
                  validateAndSubmit(values, validateForm, handleSubmit)
                }
              >
